...
void allocate() {
  probname = (char   *) malloc (16      * sizeof(char));   
...
void socp(const MultiCommoNetwork &net, const Vector &z_, Vector &p_){
  ...
  FOR(k, K){
    /*
    FOR(a, A) obj[a] = z[k][a];
    rhs[net.commoflows[k].origin]      = - net.commoflows[k].demand;
    rhs[net.commoflows[k].destination] = + net.commoflows[k].demand;

    if(mark) CPXwriteprob (env, lp, "myprob.rlp", NULL), mark = false;
    */
    ...
    /*
    rhs[net.commoflows[k].origin] = 0.0;
    rhs[net.commoflows[k].destination] = 0.0;
    */
...
void solve(const MultiCommoNetwork &net, ReducableFunction *obj){
  ...
  while(!exit_flag) {
	...
	lambda = section_search(y0, y1, robj, settings.geti("line search iterations"));
      else lambda = line_search(y0, y1, robj, settings.geti("line search iterations"));
      x1 = x0 + lambda*(x1-x0);
      y1 = y0 + lambda*(y1-y0);
    ...
    Real f_ls = f1, fsp; // for reporting
